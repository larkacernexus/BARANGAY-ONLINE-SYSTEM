...
public function index(Request $request)
{
    ...
    $staff = User::whereHas('role', function ($query) {
        ...
        ->sortBy(function ($user) {
            ...
            $lastName = $user->resident->last_name ?? '';
            $firstName = $user->resident->first_name ?? '';
        ...
        ->map(function ($user) {
            ...
            $firstName = $user->resident->first_name ?? '';
            $middleName = $user->resident->middle_name ?? '';
            $lastName = $user->resident->last_name ?? '';
    ...
    $formattedReports = $reports->map(function ($report) {
        ...
        $fullName = null;
        if ($report->user && $report->user->resident) {
            $firstName = $report->user->resident->first_name ?? '';
            $middleName = $report->user->resident->middle_name ?? '';
            $lastName = $report->user->resident->last_name ?? '';
            $fullName = trim("{$firstName} {$middleName} {$lastName}");
        }
        
        // Get assigned staff name from resident
        $assignedToName = null;
        if ($report->assignedTo && $report->assignedTo->resident) {
            $firstName = $report->assignedTo->resident->first_name ?? '';
            $middleName = $report->assignedTo->resident->middle_name ?? '';
            $lastName = $report->assignedTo->resident->last_name ?? '';
        ...
        return [
            ...
            'user' => $report->user ? [
                'id' => $report->user->id,
                'first_name' => $report->user->resident->first_name ?? null,
                'last_name' => $report->user->resident->last_name ?? null,
                'full_name' => $fullName,
...
public function store(Request $request)
{
    ...
    public function show(CommunityReport $report)
    {
        ...
        $report->load([
            ...
            'assignedTo:id,first_name,last_name,email,contact_number,position',
        ...
        $reportData = [
            ...
            'assigned_to' => $report->assignedTo ? [
                ...
                'role' => $report->assignedTo->role ? $report->assignedTo->role->name : 'No role assigned',
            ] : null,
            'resolution_notes' => $report->resolution_notes,
            'resolved_at' => $report->resolved_at ? $report->resolved_at->format('Y-m-d H:i:s') : null,
            'acknowledged_at' => $report->acknowledged_at ? $report->acknowledged_at->format('Y-m-d H:i:s') : null,
        ...
        // Get staff for assignment - Get names from residents table
$staff = User::whereHas('role', function ($query) {
        // Exclude users with "Resident" role
        $query->where('name', '!=', 'Resident');
    })
    ->whereNotNull('role_id') // Exclude users with no role assigned
    ->where('status', 'active') // Only active users
            ...
            case 'mark_resolved':
                ...
                foreach ($reports as $report) {
                    ...
                    activity()
                        ...
                        ->log("Marked community report #{$reportNumber} as resolved (bulk action)");
                }
            ...
            case 'mark_under_review':
                ...
                foreach ($reports as $report) {
                    ...
                    activity()
                        ...
                        ->event('status_updated')
                        ->log("Marked community report #{$reportNumber} as under review (bulk action)");
                }
            ...
            case 'mark_assigned':
                ...
                foreach ($reports as $report) {
                    ...
                    activity()
                        ...
                        ->event('status_updated')
                        ->log("Marked community report #{$reportNumber} as assigned (bulk action)");
                }
            ...
            case 'mark_in_progress':
                ...
                foreach ($reports as $report) {
                    ...
                    activity()
                        ...
                        ->event('status_updated')
                        ->log("Marked community report #{$reportNumber} as in progress (bulk action)");
                }
            ...
            case 'mark_pending':
                ...
                foreach ($reports as $report) {
                    ...
                    activity()
                        ...
                        ->event('status_updated')
                        ->log("Marked community report #{$reportNumber} as pending (bulk action)");
                }
            ...
            case 'delete':
                ...
                foreach ($reports as $report) {
                    ...
                    activity()
                        ...
                        ->event('deleted')
                        ->log("Deleted community report #{$reportNumber} (bulk action)");
                }
            ...
            case 'change_priority':
                ...
                foreach ($reports as $report) {
                    ...
                    activity()
                        ...
                        ->log("Changed community report #{$reportNumber} priority to {$request->priority} (bulk action)");
                }
            ...
            case 'change_urgency':
                ...
                foreach ($reports as $report) {
                    ...
                    activity()
                        ...
                        ->event('urgency_updated')
                        ->log("Changed community report #{$reportNumber} urgency to {$request->urgency} (bulk action)");
                }
            ...
            case 'assign_to':
                ...
                foreach ($reports as $report) {
                    ...
                    activity()
                        ...
                        ->event('assigned')
                        ->log("Assigned community report #{$reportNumber} to staff member (bulk action)");
                }
    ...
    /**
     * Remove evidence file from report.
     */
                ...
                if ($report->title) {
                    ...
                    foreach ($keywords as $keyword) {
                        if (strlen($keyword) > 3) {
                            ...
                            $query->orWhere('detailed_description', 'LIKE', "%{$keyword}%");
                        }
                    }
                }
            })
    ...
    /**
     * Helper: Format report for print/PDF.
     */
