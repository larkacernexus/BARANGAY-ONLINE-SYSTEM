...
export default function Users() {
    ...
    const safeUsers: LaravelPaginatedData<User> = initialUsers || {
        data: [],
        current_page: 1,
        last_page: 1,
        total: 0,
    ...
    const safeStats: UserStatsData = (stats as UserStatsData) || {
        total: 0,
        active: 0,
        inactive: 0,
        new_this_month: 0,
    ...
    const [roleFilter, setRoleFilter] = useState<string>(
        String(initialFilters.role_id || 'all'),
    );
    const [statusFilter, setStatusFilter] = useState<string>(
        String(initialFilters.status || 'all'),
    );
    const [accountTypeFilter, setAccountTypeFilter] = useState<string>(
        String(initialFilters.account_type || 'all'),
    );
    const [twoFactorFilter, setTwoFactorFilter] = useState<string>(
        String(initialFilters.two_factor || 'all'),
    );
    const [emailVerifiedFilter, setEmailVerifiedFilter] = useState<string>(
        String(initialFilters.email_verified || 'all'),
    );
    const [lastLoginFilter, setLastLoginFilter] = useState<string>(
        String(initialFilters.last_login || 'all'),
    );
    const [dateFrom, setDateFrom] = useState<string>(
        String(initialFilters.date_from || ''),
    );
    const [dateTo, setDateTo] = useState<string>(
        String(initialFilters.date_to || ''),
    );
    ...
    useEffect(() => {
        ...
        if (hasFilterChanges) reloadData();
    }, [
        debouncedSearch,
        roleFilter,
        statusFilter,
    ...
    const paginationData = {
        ...
        to: safeUsers.to || 0,
        per_page: safeUsers.per_page || parseInt(perPage) || 15,
    };
    ...
    const paginatedUsers: LaravelPaginatedData<User> = {
        data: currentUsers,
        ...
        last_page_url: buildPageUrl(paginationData.last_page),
        next_page_url:
            paginationData.current_page < paginationData.last_page
                ? buildPageUrl(paginationData.current_page + 1)
                : null,
        prev_page_url:
    ...
    const handleSelectAllOnPage = () => {
        ...
        else setSelectedUsers((prev) => [...new Set([...prev, ...pageIds])]);
            ...
            switch (operation) {
                ...
                case 'deactivate':
                    await router.post(
                        ...
                        { action: operation, user_ids: selectedUsers },
                        {
                            preserveScroll: true,
                            onSuccess: () => {
                                setSelectedUsers([]);
                                ...
                                toast.success(
                                    `${selectedUsers.length} users ${operation}d`,
                                );
                            },
                            onError: () =>
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            await router.post(
                '/admin/users/bulk-actions',
                { action: 'delete', user_ids: selectedUsers },
                {
                    preserveScroll: true,
                    onSuccess: () => {
                        setSelectedUsers([]);
            ...
            await router.post(
                ...
                },
                {
                    ...
                    onSuccess: () => {
                        ...
                        reloadData(paginationData.current_page);
                        toast.success(
                            `Role updated for ${selectedUsers.length} users`,
                        );
                    },
            ...
            <TooltipProvider>
                <div className="space-y-4 sm:space-y-6">
                    ...
                    <UsersFilters
                        ...
                        onPerPageChange={(p) => handlePerPageChange(String(p))}
                        lastLoginRange={lastLoginFilter}
                        setLastLoginRange={setLastLoginFilter}
                        searchInputRef={searchInputRef}
                    />
                    {hasActiveFilters && (
                        <div className="flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-700 dark:border-blue-800 dark:bg-blue-950/30 dark:text-blue-400">
                            <AlertCircle className="h-4 w-4 flex-shrink-0" />
                            <span className="flex-1">
                                Active filters applied. Showing{' '}
                            ...
                            <Button
                                ...
                                className="h-7 px-2 text-blue-700 hover:text-blue-800 dark:text-blue-400"
                            >
                                Clear all
                            </Button>
                        </div>
