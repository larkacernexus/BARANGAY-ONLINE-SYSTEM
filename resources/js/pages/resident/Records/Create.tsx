// pages/resident/records/create.tsx
...
import { ArrowLeft, Menu, Sparkles, AlertCircle, Eye, Upload, FileText, Shield, X, Check, Loader2 } from 'lucide-react';
...
// Extend RecordsPageProps for the create page specific props
interface CreatePageProps extends RecordsPageProps {
...
export default function DocumentCreate({ 
    ...
    // Form
    const { data, setData, post, processing, errors, reset } = useForm<RecordFormData>({
        resident_id: '',
        document_type_id: '',
        name: '',
        description: '',
    ...
    // Memoized document types with category info
    ...
    const filteredDocumentTypes = useMemo(() => {
        let types = [...allDocumentTypes];
        
        if (selectedCategory !== 'all') {
            types = types.filter(type => type.document_category_id?.toString() === selectedCategory);
        }
    ...
    const handleSubmit = (e?: React.FormEvent) => {
        ...
        post('/portal/my-records', {
            ...
            onError: (errs) => {
                ...
                if (errs.tags) {
                    setUploadError(`Tags error: ${errs.tags}`);
                } else if (errs.metadata) {
                    setUploadError(`Metadata error: ${errs.metadata}`);
                } else if (errs.reference_number) {
                    setUploadError('Reference number already exists. Please use a different one.');
                } else if (errs.password) {
                    setUploadError(`Password error: ${errs.password}`);
                } else if (errs.confirm_password) {
                    setUploadError(`Confirm password error: ${errs.confirm_password}`);
                } else {
                    setUploadError(Object.values(errs)[0] || 'Upload failed.');
                }
                setUploadProgress(0);
            },
        });
    ...
    return (
        <>
            ...
            <ResidentLayout
                ...
                hideMobileFooter={uploadStep === 'details' && isMobile}
            >
