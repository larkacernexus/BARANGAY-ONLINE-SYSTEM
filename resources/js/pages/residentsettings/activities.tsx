    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <SettingsLayout>
        <div className="space-y-4 px-1 sm:px-2">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div>
              ...
              <p className="text-xs sm:text-sm text-muted-foreground">{pagination.total} total events</p>
            </div>
            <DropdownMenu>
              ...
              <TabsList className="w-full justify-start overflow-x-auto h-10 gap-1 bg-transparent p-0">
                {[
                  ...
                  <TabsTrigger
                    key={tab.id}
                    value={tab.id}
                    className="px-3 py-1.5 text-xs sm:text-sm whitespace-nowrap data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
                  >
              ...
              <div className="flex flex-wrap gap-2 items-center text-xs">
                ...
                {activeTab !== 'all' && (
                  <Badge variant="secondary" className="gap-1 py-1">
                    {activeTab}
                    <button onClick={() => setActiveTab('all')} className="ml-1 hover:text-destructive text-base">×</button>
                  </Badge>
                )}
                {searchQuery && (
                  <Badge variant="secondary" className="gap-1 py-1">
                    "{searchQuery}"
                    <button onClick={() => setSearchQuery('')} className="ml-1 hover:text-destructive text-base">×</button>
                  </Badge>
                )}
              </div>
            )}
          ...
          <div className="space-y-2">
            {filteredActivities.length === 0 ? (
              ...
              filteredActivities.map((activity) => (
                <div
                  key={activity.id}
                  className="bg-card border rounded-lg p-3 sm:p-4 hover:bg-accent/50 transition-colors group relative"
                >
                  <div className="flex gap-2 sm:gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-1 mb-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-medium text-sm sm:text-base truncate max-w-[150px] sm:max-w-full">
                            {activity.action}
                          </span>
                          {activity.status && (
                      ...
                      <div className="flex flex-wrap gap-1">
                        ...
                        {activity.resource_id && (
                          <Badge variant="outline" className="text-[10px] sm:text-xs py-0 font-mono">
                            🔖 {activity.resource_id}
                          </Badge>
                        )}
                      </div>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => { setSelectedActivity(activity); setShowDetails(true); }}>
                          👁️ View Details
                        </DropdownMenuItem>
        ...
        <Dialog open={showDetails} onOpenChange={setShowDetails}>
          <DialogContent className="sm:max-w-2xl w-[95vw] p-4">
            <DialogHeader>
            ...
            {selectedActivity && (
              <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs sm:text-sm">
                  <div><span className="text-muted-foreground">ID:</span> <span className="font-mono break-all">{selectedActivity.id}</span></div>
                  <div><span className="text-muted-foreground">Type:</span> {selectedActivity.type}</div>
                  <div><span className="text-muted-foreground">Action:</span> {selectedActivity.action}</div>
                  <div><span className="text-muted-foreground">Status:</span> {selectedActivity.status}</div>
                  <div className="sm:col-span-2"><span className="text-muted-foreground">Description:</span> {selectedActivity.description}</div>
                  <div className="sm:col-span-2"><span className="text-muted-foreground">Time:</span> {new Date(selectedActivity.timestamp).toLocaleString()}</div>
                  {selectedActivity.ip_address && <div><span className="text-muted-foreground">IP:</span> {selectedActivity.ip_address}</div>}
                  {selectedActivity.device && <div><span className="text-muted-foreground">Device:</span> {selectedActivity.device}</div>}
                  {selectedActivity.resource_id && <div><span className="text-muted-foreground">Ref:</span> {selectedActivity.resource_id}</div>}
                </div>
