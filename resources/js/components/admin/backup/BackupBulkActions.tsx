// BackupBulkActions.tsx - Updated to accept string[] and BulkOperationType
...
import type { SelectionMode, SelectionStats, BulkOperationType } from '@/types/admin/backup/backup';
...
interface BackupBulkActionsProps {
  ...
  onSelectAll: () => void;
  onBulkOperation: (operation: BulkOperationType) => void; // Changed from string to BulkOperationType
...
export default function BackupBulkActions({
  ...
  const bulkActions = [
    {
      ...
      onClick: () => onBulkOperation('export' as BulkOperationType),
      variant: 'ghost' as const
    },
    {
      label: 'Print',
      icon: <Printer className="h-3.5 w-3.5 mr-2" />,
      onClick: () => onBulkOperation('print' as BulkOperationType),
      variant: 'ghost' as const
    },
    {
  ...
  // Safely access selectionStats properties with fallbacks
  const protectedCount = selectionStats?.protectedCount ?? 0;
  const formattedSize = selectionStats?.formattedSize ?? selectionStats?.total_size ?? '0 B';
  const largestSize = selectionStats?.largestSize ?? 0;
  const fullCount = selectionStats?.fullCount ?? 0;
  const databaseCount = selectionStats?.databaseCount ?? 0;
  const filesCount = selectionStats?.filesCount ?? 0;
  const total = selectionStats?.total ?? 0;

  // Format largest size safely
  const formattedLargestSize = typeof largestSize === 'number' 
    ? formatBytes(largestSize) 
    : '0 B';
    ...
    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/10 dark:to-indigo-900/10 border border-blue-200 dark:border-blue-800 rounded-lg p-4 shadow-sm">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div ref={bulkActionRef} className="flex items-center gap-2">
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => onBulkOperation('download' as BulkOperationType)}
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => onBulkOperation('restore' as BulkOperationType)}
                  className="h-8 dark:border-gray-700"
                  disabled={isPerformingBulkAction}
                >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => onBulkOperation((protectedCount > 0 ? 'unprotect' : 'protect') as BulkOperationType)}
                  className="h-8 dark:border-gray-700"
                  disabled={isPerformingBulkAction}
                >
                  {protectedCount > 0 ? (
              ...
              <TooltipContent>
                {protectedCount > 0 ? 'Remove protection' : 'Add protection'}
              </TooltipContent>
            </Tooltip>
            ...
            <Button
              variant="outline"
              className="h-8 dark:border-gray-700"
              onClick={() => onBulkOperation('exit' as BulkOperationType)}
              disabled={isPerformingBulkAction}
            >
        ...
        <div className="mt-3 pt-3 border-t border-blue-100 dark:border-blue-800">
          ...
          <div className="mt-2 grid grid-cols-3 gap-2 text-xs text-gray-500 dark:text-gray-400">
            <div className="flex items-center gap-1">
              ...
              <span>{fullCount} full system</span>
            </div>
            <div className="flex items-center gap-1">
              <ServerCog className="h-3 w-3 text-green-500" />
              <span>{databaseCount} database</span>
            </div>
            <div className="flex items-center gap-1">
              <FileLock className="h-3 w-3 text-purple-500" />
              <span>{filesCount} files only</span>
            </div>
          </div>
        </div>
      )}
